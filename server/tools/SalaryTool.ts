...
export interface SalaryLookupInput {
  /** Defaults to the requesting employee when omitted. */
  employeeId?: string;
}

export interface SalaryLookupOutput {
  employeeId: string;
  records: GasRecord[];
}

export const salaryTool: BaseTool<SalaryLookupInput, SalaryLookupOutput> = {
  name: "salary_lookup",
  description:
    "Looks up salary payment records for an employee, including bank/payment details. Defaults to the requesting employee; looking up someone else requires HOD/HR/Admin.",
  parameters: {
    type: "object",
    properties: {
  ...
  async execute(input, context) {
    ...
    const res = await getGasClient().call<GasRecord>("getSalaryRecords", { employeeId: target });
    return { employeeId: target, records: unwrapGasList(res) };
  },
};

toolRegistry.register(salaryTool);
