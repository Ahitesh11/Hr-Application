...
import { LogIn, Lock, Loader2, Eye, EyeOff, Users, TrendingUp, Clock, CheckCircle } from "lucide-react";
  ...
  return (
    <div className="min-h-screen flex bg-stone-50">
      ...
      <div
        ...
        style={{ background: "linear-gradient(135deg, #283709 0%, #3a4e0f 40%, #4d6515 75%, #618016 100%)" }}
      >
        {/* Decorative circles */}
        <div className="absolute top-[-100px] right-[-100px] w-[380px] h-[380px] rounded-full"
          style={{ background: "rgba(255,255,255,0.04)" }} />
        <div className="absolute bottom-[-80px] left-[-80px] w-[320px] h-[320px] rounded-full"
          style={{ background: "rgba(255,255,255,0.04)" }} />
        ...
        <div className="relative z-10 flex-1 flex flex-col justify-center">
          <p className="text-white/50 text-xs font-bold uppercase tracking-[0.2em] mb-3"></p>
          <h2 className="text-4xl font-black text-white leading-tight mb-4">
            Manage Your<br />
            <span style={{ color: "#a1bb59" }}>Workforce</span><br />
            Efficiently
          </h2>
          ...
          <div className="grid grid-cols-2 gap-3">
            {features.map((f, i) => (
              <div key={i} className="flex items-center gap-2.5 rounded-2xl px-4 py-3"
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <input
                type="text"
                value={employeeId}
                onChange={(e: { target: { value: string } }) => setEmployeeId(e.target.value)}
                className="w-full px-4 py-3.5 bg-pink-50 border-2 border-pink-100 rounded-2xl text-sm font-bold text-slate-900 placeholder:text-slate-300 placeholder:font-normal outline-none transition-all"
                style={{ "--tw-ring-color": "#4d6515" } as any}
                onFocus={e => { e.currentTarget.style.borderColor = "#4d6515"; e.currentTarget.style.background = "#fff"; }}
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full pl-4 pr-12 py-3.5 bg-pink-50 border-2 border-pink-100 rounded-2xl text-sm font-bold text-slate-900 placeholder:text-slate-300 placeholder:font-normal outline-none transition-all w-full"
                  onFocus={e => { e.currentTarget.style.borderColor = "#4d6515"; e.currentTarget.style.background = "#fff"; }}
                  onBlur={e => { e.currentTarget.style.borderColor = ""; e.currentTarget.style.background = ""; }}
                  placeholder="••••••••••"
                  required
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-4 text-white font-black text-sm rounded-2xl transition-all flex items-center justify-center gap-2.5 disabled:opacity-70 disabled:cursor-not-allowed mt-2 active:scale-[0.98]"
              style={{ background: isLoading ? "#3a4e0f" : "#4d6515", boxShadow: "0 8px 24px rgba(77,101,21,0.35)" }}
              onMouseEnter={e => { if (!isLoading) e.currentTarget.style.background = "#3a4e0f"; }}
              onMouseLeave={e => { if (!isLoading) e.currentTarget.style.background = "#4d6515"; }}
            >
              {isLoading ? (
